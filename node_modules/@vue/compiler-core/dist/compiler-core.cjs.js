	...
	for (let i = 0; i < children.length; i++) {
		...
		if (child.type === 1 && child.tagType === 0) {
			...
			if ((doNotHoistNode ? 0 : getConstantType(child, context)) >= 2) {
				if (child.codegenNode.type === 14 && child.codegenNode.arguments.length > 0) child.codegenNode.arguments.push(`-1 /* ${_vue_shared.PatchFlagNames[-1]} */`);
				toCache.push(child);
				continue;
			}
		}
...
function genVNodeCall(node, context) {
	...
	let patchFlagString;
	if (patchFlag) if (patchFlag < 0) patchFlagString = patchFlag + ` /* ${_vue_shared.PatchFlagNames[patchFlag]} */`;
	else patchFlagString = patchFlag + ` /* ${Object.keys(_vue_shared.PatchFlagNames).map(Number).filter((n) => n > 0 && patchFlag & n).map((n) => _vue_shared.PatchFlagNames[n]).join(`, `)} */`;
...
function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
	...
	let slots = createObjectExpression(slotsProperties.concat(createObjectProperty(`_`, createSimpleExpression(slotFlag + ` /* ${_vue_shared.slotFlagsText[slotFlag]} */`, false))), loc);
				...
				if (exp) if (isVBind) {
					...
					if (mergeArgs.some((arg) => {
						if (arg.type === 15) return arg.properties.some(({ key }) => {
							...
							return key.content !== "class" && key.content !== "style" && !(0, _vue_shared.isOn)(key.content);
						});
						else return true;
	...
	if (node.type === 0 || node.type === 1 || node.type === 11 || node.type === 10) return () => {
		...
		for (let i = 0; i < children.length; i++) {
			...
			if (isText(child) || child.type === 8) {
				...
				if (!context.ssr && getConstantType(child, context) === 0) callArgs.push(`1 /* ${_vue_shared.PatchFlagNames[1]} */`);
				children[i] = {
					type: 12,
					content: child,
					loc: child.loc,
