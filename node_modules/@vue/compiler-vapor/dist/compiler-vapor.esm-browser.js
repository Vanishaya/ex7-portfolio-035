...
function decodeBase64(input) {
	const binary = typeof atob === "function" ? atob(input) : typeof Buffer$1.from === "function" ? Buffer$1.from(input, "base64").toString("binary") : new Buffer$1(input, "base64").toString("binary");
		...
		fillOptionalPropertiesForTSESLint(node) {
			switch (node.type) {
				case "ExpressionStatement":
					node.directive != null || (node.directive = void 0);
					return;
				...
				case "AssignmentPattern":
				case "ObjectPattern":
					node.decorators != null || (node.decorators = []);
					node.optional != null || (node.optional = false);
					node.typeAnnotation != null || (node.typeAnnotation = void 0);
					return;
				case "TSParameterProperty":
					node.accessibility != null || (node.accessibility = void 0);
					node.decorators != null || (node.decorators = []);
					node.override != null || (node.override = false);
					node.readonly != null || (node.readonly = false);
					node.static != null || (node.static = false);
					return;
				...
				case "ClassMethod":
				case "ClassPrivateMethod":
					node.declare != null || (node.declare = false);
					node.returnType != null || (node.returnType = void 0);
					node.typeParameters != null || (node.typeParameters = void 0);
					return;
				case "Property":
					node.optional != null || (node.optional = false);
					return;
				case "TSMethodSignature":
				case "TSPropertySignature": node.optional != null || (node.optional = false);
				...
				case "AccessorProperty":
					node.declare != null || (node.declare = false);
					node.definite != null || (node.definite = false);
					node.readonly != null || (node.readonly = false);
					node.typeAnnotation != null || (node.typeAnnotation = void 0);
				...
				case "MethodDefinition":
					...
					node.optional != null || (node.optional = false);
					return;
				case "ClassExpression": node.id != null || (node.id = null);
				case "ClassDeclaration":
					node.abstract != null || (node.abstract = false);
					node.declare != null || (node.declare = false);
					node.decorators != null || (node.decorators = []);
					node.implements != null || (node.implements = []);
					node.superTypeArguments != null || (node.superTypeArguments = void 0);
					node.typeParameters != null || (node.typeParameters = void 0);
					return;
				...
				case "VariableDeclarator":
					...
					return;
				case "TSEnumDeclaration":
					node.const != null || (node.const = false);
					node.declare != null || (node.declare = false);
					return;
				case "TSEnumMember":
					node.computed != null || (node.computed = false);
					return;
				case "TSImportType":
					node.qualifier != null || (node.qualifier = null);
					node.options != null || (node.options = null);
					return;
				...
				case "TSMappedType":
					node.optional != null || (node.optional = false);
					node.readonly != null || (node.readonly = void 0);
					return;
				case "TSModuleDeclaration":
					node.declare != null || (node.declare = false);
					node.global != null || (node.global = node.kind === "global");
					return;
				case "TSTypeParameter":
					node.const != null || (node.const = false);
					node.in != null || (node.in = false);
					node.out != null || (node.out = false);
					return;
			}
		}
...
const isMemberExpressionNode = (exp, context) => {
	try {
		let ret = exp.ast || (0, import_lib.parseExpression)(getExpSource(exp), { plugins: context.expressionPlugins ? [...context.expressionPlugins, "typescript"] : ["typescript"] });
...
function baseParse(input, options) {
	...
	if (currentOptions.decodeEntities) console.warn("[@vue/compiler-core] decodeEntities option is passed but will be ignored in non-browser builds.");
...
function resolveExpression(exp, isComponent) {
	if (!exp.isStatic) {
		...
		if (value !== null) return createSimpleExpression(value, true, exp.loc);
	}
	return exp;
}
...
function transformNode(context) {
	...
	for (const nodeTransform of nodeTransforms) {
		...
		if (onExit) if (isArray$1(onExit)) exitFns.push(...onExit);
...
var import_source_map = require_source_map();
...
function codeFragmentToString(code, context) {
	...
	let map;
	if (sourceMap) {
		map = new import_source_map.SourceMapGenerator();
	...
	for (let frag of code) {
		...
		if (map) {
			...
			if (newlineIndex === -3) advancePositionWithMutation(pos, code);
			else {
...
function analyzeExpressions(expressions) {
	...
	for (const exp of expressions) {
		...
		walkIdentifiers(exp.ast, (currentNode, parent, parentStack) => {
			...
			} else if (!parentStack.some(isMemberExpression)) registerVariable(currentNode.name, exp, true, {
	...
	if (values) {
		values.forEach((value, index) => {
			...
			if (value && value.content.trim()) {
				...
				} else if (isFnExpression(value, context.options)) exp = genExpression(value, context);
				else {
...
function parseValueDestructure(value, context) {
	...
	if (value) {
		...
		if (value.ast) walkIdentifiers(value.ast, (id, _, parentStack, ___, isLocal) => {
			if (isLocal) {
				let path = "";
				let isDynamic = false;
				let helper;
...
function processInlineHandlers(props, context) {
	...
	if (isArray$1(staticProps)) for (let i = 0; i < staticProps.length; i++) {
...
function genStaticProps(props, context, dynamicProps) {
	...
	const getStaticPropKeyName = (prop) => {
		...
		return (prop.handler ? toHandlerKey(camelize(prop.key.content)) : prop.key.content) + handlerModifierPostfix;
	};
...
function transformComponentElement(node, propsResult, staticKey, singleRoot, context, isDynamicComponent, useCreateElement) {
	...
	if (!dynamicComponent && !useCreateElement) {
		...
		if (asset) {
			if (context.selfName && capitalize(camelize(tag)) === context.selfName) tag += `__self`;
...
function resolveDynamicComponent(node) {
	...
	if (!isProp) return;
	if (isProp.type === 6) return isProp.value && createSimpleExpression(isProp.value.content, true);
	else return isProp.exp || extend(createSimpleExpression(`is`, false, isProp.arg.loc), { ast: null });
}
	...
	for (const prop of props) {
		...
		if (result) {
			...
			if (isComponent && !result.key.isStatic) {
				pushMergeArg();
				dynamicArgs.push(extend(resolveDirectiveResult(result), { kind: 1 }));
...
function dedupeProperties(results) {
	...
	for (const result of results) {
		...
		if (existing && existing.handler === prop.handler) {
			if (name === "style" || name === "class" || prop.handler) mergePropValues(existing, prop);
		} else {
...
const transformVText = (dir, node, context) => {
	let { exp, loc } = dir;
	if (!exp) {
		context.options.onError(createDOMCompilerError(56, loc));
		exp = EMPTY_EXPRESSION;
	}
	if (node.children.length) {
...
const transformVShow = (dir, node, context) => {
	const { exp, loc } = dir;
	if (!exp) {
		context.options.onError(createDOMCompilerError(62, loc));
		return;
	}
	if (node.tagType === 2) {
...
const transformTemplateRef = (node, context) => {
	...
	else value = dir.value ? createSimpleExpression(dir.value.content, true, dir.value.loc) : EMPTY_EXPRESSION;
	return () => {
		const id = context.reference();
...
const transformVModel = (dir, node, context) => {
	const { exp, arg } = dir;
	if (!exp) {
		context.options.onError(createCompilerError(41, dir.loc));
		return;
	}
	const rawExp = exp.loc.source;
	const bindingType = context.options.bindingMetadata[rawExp];
	if (bindingType === "props" || bindingType === "props-aliased") {
		context.options.onError(createCompilerError(44, exp.loc));
		return;
	}
	...
	if (!expString.trim() || !isMemberExpression$1(exp, context.options) && !maybeRef) {
		context.options.onError(createCompilerError(42, exp.loc));
		return;
	}
	...
	if (dir.arg) context.options.onError(createDOMCompilerError(59, dir.arg.loc));
	const { tag } = node;
...
function getSiblingIf(context, reverse) {
	...
	while (reverse ? --i >= 0 : ++i < siblings.length) {
		sibling = siblings[i];
		if (!isCommentOrWhitespace(sibling)) break;
	}
...
function processIf(node, dir, context) {
	if (dir.name !== "else" && (!dir.exp || !dir.exp.content.trim())) {
		...
		context.options.onError(createCompilerError(28, dir.loc));
...
function processFor(node, dir, context) {
	if (!dir.exp) {
		context.options.onError(createCompilerError(31, dir.loc));
		return;
	}
	const parseResult = dir.forParseResult;
	if (!parseResult) {
		context.options.onError(createCompilerError(32, dir.loc));
		return;
	}
...
const transformVSlot = (node, context) => {
	...
	else if (!isComponent && dir) context.options.onError(createCompilerError(40, dir.loc));
};
...
const transformTransition = (node, context) => {
	if (node.type === 1 && node.tagType === 1) {
		if (isTransitionTag(node.tag)) return postTransformTransition(node, context.options.onError, hasMultipleChildren);
	}
};
...
function compile(source, options = {}) {
	...
	return generate(transform(ast, extend({}, resolvedOptions, {
		nodeTransforms: [
			...nodeTransforms,
			...[transformTransition],
