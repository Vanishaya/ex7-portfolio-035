		...
		switch (type) {
			...
			default: if (shapeFlag & 1) if ((domType !== 1 || vnode.type.toLowerCase() !== node.tagName.toLowerCase()) && !isTemplateNode(node)) nextNode = onMismatch();
				...
				if (vnode.type.__vapor) {
					...
					getVaporInterface(parentComponent, vnode).hydrate(vnode, node, container, null, parentComponent, parentSuspense, () => {
						if (vnode.dirs) {
...
var createHook = /* @__NO_SIDE_EFFECTS__ */ (lifecycle) => (hook, target = currentInstance) => {
...
var normalizeRef = ({ ref, ref_key, ref_for }, i = currentRenderingInstance) => {
...
*/
var Transition = /* @__PURE__ */ decorate$1$1((props, { slots }) => h(BaseTransition, resolveTransitionProps(props), slots));
/**
...
var TransitionGroup = /* @__PURE__ */ decorate$2({
...
//#endregion
//#region node_modules/@vue/runtime-vapor/dist/runtime-vapor.esm-bundler.js
/**
...
var deferredHydrationBoundaryDepth = 0;
...
function enterHydration(node) {
	...
	if (!prevHydrationEnabled) setIsHydratingEnabled$1(true);
	const prev = setIsHydrating(true);
	const prevHydrationNode = currentHydrationNode;
	...
	return () => {
		currentHydrationNode = prevHydrationNode;
		setIsHydrating(prev);
		if (!prevHydrationEnabled) setIsHydratingEnabled$1(false);
	};
}
var adoptTemplate;
...
function finalizeHydrationBoundary(close) {
	let node = currentHydrationNode;
	if (!close || !node || node === close) return;
	let cur = node;
...
function propGetter(rawProps, key) {
	...
	if (dynamicSources) {
		...
		while (i--) {
			...
			if (hasOwn(source, key)) return isInteropEnabled && dynamicSources[interopKey] || isOn(key) && isFunction(dynamicSources[i]) ? source[key] : resolveSource(source[key]);
		}
	}
...
var applyTransitionHooks;
var applyTransitionLeaveHooks;
var isTransitionEnabled = false;
...
var currentHydratingSlotBoundaryState = null;
...
var VaporTransition = /* @__PURE__ */ decorate$1((props, { slots, expose }) => {
	expose();
...
var transitionTypeMap = /* @__PURE__ */ new WeakMap();
...
var getTransitionHooksContext = (block, props, state, instance, postClone) => {
...
var VaporKeepAlive = /* @__PURE__ */ withKeepAliveEnabled(/* @__PURE__ */ defineVaporComponent({
	name: "VaporKeepAlive",
	__isKeepAlive: true,
	props: {
		include: [
...
var vaporSlotWrappersCache = /* @__PURE__ */ new WeakMap();
...
var collectedVdomSlotVNodes = /* @__PURE__ */ new WeakMap();
...
function createCollectedVDOMSlotVNode(component, rawProps, slots) {
	const vnode = createVNode(component, rawProps && extend({}, new Proxy(rawProps, rawPropsProxyHandlers)), slots === EMPTY_OBJ ? null : new Proxy(slots, vaporSlotsProxyHandler));
	vnode.scopeId = getCurrentScopeId() || null;
	vnode.slotScopeIds = currentSlotScopeIds;
	return vnode;
}
var rendererBridgeCache = /* @__PURE__ */ new WeakMap();
/**
...
var vnodeHookStateMap = /* @__PURE__ */ new WeakMap();
...
var isInternalSlotKey = (key) => key === "_" || key === "_ctx" || key === "$stable";
var interopSlotsSourceHandlers = {
	get(target, key) {
...
var VaporTeleport = /* @__PURE__ */ enableTeleport(VaporTeleportImpl);
...
var vaporCustomElementHydrates = /* @__PURE__ */ new WeakMap();
/* @__NO_SIDE_EFFECTS__ */
		...
		try {
			if (emptyLocalRange && newLength) {
				...
				if (resolvedAnchor) {
					...
					parentAnchor = markHydrationAnchor(/* @__PURE__ */ createComment("for"));
					pendingHydrationAnchor = true;
					...
					if (!isComment(parentAnchor, "]")) throw new Error(`v-for fragment anchor node was not found. this is likely a Vue internal bug.`);
		...
		if (_isString || _isRef) {
			const doSet = () => {
				if (refFor) {
					...
					existing = _isString ? canSetSetupRef(ref) ? setupState[ref] : refs[ref] : canSetRef(ref) || !refKey ? ref.value : refs[refKey];
					if (!isArray(existing)) {
						existing = [refValue];
						if (_isString) {
							refs[ref] = existing;
...
var isForHydrationAnchorResolverRegistered = false;
var currentForHydrationContainer;
function ensureForHydrationAnchorResolver() {
	...
	setForHydrationAnchorResolver((hydrationStart, anchorNode) => {
		...
		const parentAnchor = markHydrationAnchor(/* @__PURE__ */ createComment("for"));
...
var VaporTransitionGroup = /* @__PURE__ */ decorate(/* @__PURE__ */ defineVaporComponent({
